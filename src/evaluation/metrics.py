...
def calculate_grid_auroc(model, data_loader, device, max_batches=None):
    model.eval()
    ...
    if max_batches is not None:
        ...
        print(f"Processing evaluation stream ({total_batches} batches)...")
    ...
    print("\nCalculating stable AUROC curve metrics on CPU...")
    ...
    metrics = {
        ...
        "Foreground Macro AUROC (Crack & Pothole)": (float(per_class_auroc[0]) + float(per_class_auroc[1])) / 2.0,
